...
def evaluate_graph(model: HookedTransformer, graph: Graph, dataloader: DataLoader,
                   ...
                   intervention_dataloader: Optional[DataLoader] = None, skip_clean: bool = True) -> Union[
    ...
    in_graph_matrix = graph.in_graph.to(device=model.cfg.device_map['embed'], dtype=model.cfg.dtype)

    # same thing but for neurons
    if graph.neurons_in_graph is not None:
        neuron_matrix = graph.neurons_in_graph.to(device=model.cfg.device_map['embed'], dtype=model.cfg.dtype)
    ...
    if model.cfg.use_normalization_before_and_after:
        ...
        attention_head_mask = torch.zeros((graph.n_forward, model.cfg.n_layers), device=model.cfg.device_map['embed'],
    ...
    def make_input_construction_hook(activation_matrix, in_graph_vector, neuron_matrix):
        def input_construction_hook(activations, hook):
            ...
            update = update.to(activations.device)
            activations += update
            return activations
    ...
    for clean, corrupted, label in dataloader:
        ...
        (fwd_hooks_corrupted, fwd_hooks_clean, _), activation_difference = make_hooks_and_matrices(model, graph,
                                                                                                   len(clean), n_pos,
                                                                                                   None)
